...
function RenderMessage({ content }) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('# ')) {
      elements.push(
        <h2 key={key++} style={{ fontFamily: 'Playfair Display, serif', fontStyle: 'italic', fontSize: '1.3rem', color: '#00C8D4', margin: '0.5rem 0 0.75rem', fontWeight: 700 }}>
          {line.replace('# ', '')}
        </h2>
      );
    } else if (line.startsWith('## ')) {
      elements.push(
        <h3 key={key++} style={{ fontFamily: 'Syne, sans-serif', fontSize: '0.9rem', fontWeight: 700, color: '#F0F4F8', margin: '1rem 0 0.4rem', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
          {line.replace('## ', '')}
        </h3>
      );
    } else if (line.startsWith('**') && line.endsWith('**')) {
      elements.push(
        <p key={key++} style={{ fontWeight: 600, color: '#D0D8E4', margin: '0.25rem 0', fontSize: '0.875rem' }}>
          {line.replace(/\*\*/g, '')}
        </p>
      );
    } else if (/^\*\*.*\*\*/.test(line)) {
      ...
      elements.push(
        <p key={key++} style={{ margin: '0.2rem 0', fontSize: '0.875rem', color: '#D0D8E4' }}>
          {parts.map((p, pi) =>
            p.startsWith('**') ? <strong key={pi} style={{ color: '#F0F4F8' }}>{p.replace(/\*\*/g, '')}</strong> : p
          )}
        </p>
      );
      ...
      elements.push(
        <div key={key++} style={{ display: 'flex', gap: '8px', margin: '3px 0', paddingLeft: '4px' }}>
          <span style={{ color: '#00C8D4', marginTop: '2px', flexShrink: 0 }}>•</span>
          <span style={{ fontSize: '0.9rem', color: '#D0D8E4', lineHeight: 1.6 }}>{line.replace(/^[-*]\s/, '')}</span>
        </div>
      );
      ...
      elements.push(
        <div key={key++} style={{ display: 'flex', gap: '10px', margin: '5px 0', paddingLeft: '4px' }}>
          <span style={{ color: '#00C8D4', fontWeight: 700, fontSize: '0.85rem', minWidth: '20px', flexShrink: 0 }}>{num}.</span>
          <span style={{ fontSize: '0.9rem', color: '#D0D8E4', lineHeight: 1.6 }}>{line.replace(/^\d+\.\s/, '')}</span>
        </div>
      );
    } else if (line.trim() === '') {
      ...
      elements.push(
        <p key={key++} style={{ margin: '0.2rem 0', fontSize: '0.9rem', color: '#D0D8E4', lineHeight: 1.7 }}>
          {line}
        </p>
      );
    }
...
function CopyButton({ text }) {
  ...
  return (
    <button onClick={handleCopy} title="Copy" style={{
      ...
      color: copied ? '#00E5C3' : '#8B9AAB', padding: '4px',
    ...
    <div style={{
      ...
      <div style={{
        ...
        background: isUser
          ? `rgba(0,200,212,0.12)`
          : '#1A1F24',
        border: `1px solid ${isUser ? 'rgba(0,200,212,0.25)' : 'rgba(255,255,255,0.06)'}`,
        borderRadius: isUser ? '18px 18px 4px 18px' : '4px 18px 18px 18px',
        padding: '0.875rem 1.1rem',
      }}>
        {isUser ? (
          <p style={{ margin: 0, fontSize: '0.9rem', color: '#F0F4F8', lineHeight: 1.6 }}>{msg.content}</p>
        ) : (
          <>
            <RenderMessage content={msg.content} />
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '0.5rem', paddingTop: '0.5rem', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
      ...
      {isUser && (
        <div style={{
          ...
          fontFamily: 'Syne', fontWeight: 800, fontSize: '0.75rem', color: '#00C8D4',
        }}>
          U
        </div>
      )}
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, maxWidth: '900px', width: '100%', margin: '0 auto', padding: '1.5rem 1rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        <div>
          ...
          <h1 style={{ fontFamily: 'Playfair Display, serif', fontStyle: 'italic', fontSize: 'clamp(1.6rem, 4vw, 2.2rem)', fontWeight: 700, color: '#F0F4F8', margin: 0 }}>
            Chef <span style={{ color: '#00C8D4' }}>AI</span> Assistant
          </h1>
        </div>
          ...
          {MODES.map(m => {
            ...
            return (
              <button
                ...
                style={{
                  background: active ? m.accent : 'rgba(255,255,255,0.02)',
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                  <Icon size={16} color={active ? m.color : '#8B9AAB'} />
                  <span style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '0.82rem', color: active ? m.color : '#8B9AAB' }}>
                    {m.label}
                  </span>
                </div>
                <p style={{ margin: 0, fontSize: '0.75rem', color: active ? 'rgba(240,244,248,0.7)' : '#8B9AAB', lineHeight: 1.4 }}>
                  {m.desc}
                </p>
              </button>
            );
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '0.75rem', color: '#8B9AAB' }}>· Llama 3 via Groq</span>
            </div>
            <button
              onClick={clearChat}
              title="Clear chat"
              style={{ background: 'none', border: 'none', color: '#8B9AAB', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.75rem', fontFamily: 'DM Sans', padding: '4px 8px', borderRadius: '6px', transition: 'all 0.15s' }}
            >
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '1.25rem', scrollbarWidth: 'thin', scrollbarColor: '#1A1F24 transparent' }}>
            ...
            {loading && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '1rem' }}>
                ...
                <div style={{ background: '#1A1F24', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '4px 18px 18px 18px', padding: '0.875rem 1.1rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <Loader2 size={14} color={mode.color} className="animate-spin-slow" />
                  <span style={{ fontSize: '0.875rem', color: '#8B9AAB' }}>Chef AI is thinking...</span>
                </div>
              </div>
            )}
            ...
            {/* Error */}
            {error && (
              <div style={{ background: 'rgba(255,107,107,0.08)', border: '1px solid rgba(255,107,107,0.2)', borderRadius: '10px', padding: '0.75rem 1rem', marginBottom: '1rem', fontSize: '0.875rem', color: '#FF6B6B' }}>
                ⚠️ {error}
              </div>
            )}
            ...
            {messages.length <= 1 && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '0.75rem' }}>
                {activeMode === 'chat' && [
                  ...
                  <button key={s} onClick={() => setInput(s)} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '999px', padding: '5px 12px', fontSize: '0.78rem', color: '#8B9AAB', cursor: 'pointer', transition: 'all 0.15s', fontFamily: 'DM Sans', whiteSpace: 'nowrap' }}>
                    {s}
                  </button>
                ))}
                {activeMode === 'generate' && [
            ...
            <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-end' }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: input.trim() && !loading ? mode.color : 'rgba(255,255,255,0.06)',
              ...
              >
                {loading
                  ...
                  : <Send size={18} color={input.trim() ? '#080B0F' : '#8B9AAB'} />
                }
              </button>
            </div>
            <p style={{ margin: '0.5rem 0 0', fontSize: '0.72rem', color: '#8B9AAB', textAlign: 'center' }}>
