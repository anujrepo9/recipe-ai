...
Make it mouth-watering and evocative. No bullet points — flowing prose only. Max 60 words.`;
    ...
    } else if (mode === 'ai-detail') {
      const { recipeName, cuisine, location, spiceLevel, cookingTime, rating, ingredients } = context || {};
      systemPrompt = `You are ChefAI, a professional chef and culinary writer. Generate a comprehensive, detailed recipe profile.
...
Available Ingredients: ${Array.isArray(ingredients) ? ingredients.join(', ') : (ingredients || 'various')}
...
Keep the tone confident, warm, and practical. Total response: 350-450 words.`;
    ...
    const response = await fetch(GROQ_API_URL, {
      ...
      body: JSON.stringify({
        ...
        max_tokens: (mode === 'generate' || mode === 'ai-detail') ? 1000 : 500,
        stream: false,
      }),
    });
